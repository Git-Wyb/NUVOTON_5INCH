...
static uint8_t    *tx_buff_uart2,*tx_buff_uart2_noshift;//[TX_BUFF_UART2_MAX_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
uint16_t  tx_buff_uart2_num=0;
static uint8_t    *rrx_buff_uart2,*rrx_buff_uart2_noshift;//[RX_BUFF_UART2_MAX_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
...
//下载程序的口初始化
void download_gpio_set_output(void)  //配置成输出 DOWNLOAD_HEX_INIT(void)
{
...
void download_gpio_set_input(void)   //配置成输入DOWNLOAD_HEX_IN(void) 
{
...
//0 失败
//1 成功
uint8_t decode_ack(uint8_t *buff)   // 1 表示成功 0 失败  GET_RL78_ACK(uint8_t x_ACK_LONG)
{
...
{
	uint8_t i;
	//Clear_sdram(0X41);//清理第一层和液晶显示区域 20171009什么也不清 目前只支持750K大小的HEX 多了画面要出错 
...
static uint8_t extract_hex(FIL  file)   //FIL fsrc;    COPY_HEX_TO_SDRAM
{
	...
	{
		//feed_iwdg();//喂狗
		res = f_read(&file, buff, 3, (void *)&numOfReadBytes_hex);//读取行数信息
		...
		res = f_read(&file, (uint8_t *)buff, Tp_Reallong, (void *)&numOfReadBytes_hex);//读取行数信息
		if((numOfReadBytes_hex == 0) || (res != FR_OK))
		{
			return 0;
		}
		...
		if(intel_hex.data_type == 0x01)//文件的结束
		{
			download.hex_len =Tp_sub;//0x10000;//Tp_sub;//0X1000;// Tp_sub;
			return 1;
		}
		else if(intel_hex.data_type == 0x03)//文件的开头
		{
			download.hex_len =0;
			Tp_sub = 0;
		}
		else if(intel_hex.data_type == 0)//只有数据才要
		{
...
int copy_download_hex_to_sdram(void)  //0 成功 －1 失败
{
//	uint8_t ack;
	FRESULT res;
	//	int type = 0;
...
//	int ret = 0;
  ...
  char filename_new_hex[60]="3:/BELMONT_HEX/TBXXXXXX.HEX";//这个是用来存储带文件夹的文件路径的
  FILINFO fno = {0};
...
//feed_iwdg();//喂狗
		...
		{
			if((strstr(fn, ".hex")) || (strstr(fn, ".HEX")))//字符串匹配
			{				
    ...
    if(download.task)
   	{
			...
			Download_code.val = 0;
		    ack=copy_download_hex_to_sdram(); //int copy_download_hex_to_sdram(void)  //0 成功 －1 失败
        if(ack==0)
        {    
		      download.task=0;
...
over_flag:
	...
	//	Clear_sdram(0X0F);
       ...
       //////////HAL_UART_MspDeInit(&UART2_Handler);	//没加这个下面初始化过不去，uart2_init一直有中断接收
		  download.bps = 38400;
