...
void convet_hex_to_hex_asccii(uint8_t x_FSC,uint8_t *x_BUFF)  //0x A1  תΪ'A' '1' DIVIDED_DATA_HEX2BYTE(uint8_t x_FSC,uint8_t *x_BUFF)
{
	if((x_FSC/16)>9)
	{
...
int8_t convert_1_hex_to_hex_ascii(int8_t a)  //�ʺ�һλ�ģ���С��0x10  �� 0x0A ��Ϊ'A'    0x01��Ϊ'1'
{
	if(a>=10)
	{
	   return ('A'+(a-10));
...
uint16_t convert_1_dec_ascii_to_hex(uint8_t  *x_data)//�õ����� CHANGE_DECDATA_1BYTE
{
	static uint16_t Tp_long=0xffff;
	Tp_long=0xffff;
...
uint16_t convet_3_dec_asccii_to_hex(uint8_t  *x_data)//�õ����� CHANGE_DECDATA_3BYTE
{
	static uint16_t Tp_long=0xffff;
	Tp_long=0xffff;
...
uint16_t convert_2_dec_ascii_to_hex(uint8_t  *x_data)//�õ�����  CHANGE_DECDATA_2BYTE
{
	static uint16_t Tp_long=0xffff;
	Tp_long=0xffff;
...
void code_protocol_error(int8_t type,uint8_t Value_1,uint16_t Value_2,uint8_t cnt)  //code_protocol_error(ERROR_TYPE_U1,(U1A01_ERROR>>8),(int16_t)U1A01_ERROR,ERROR_TYPE_SEND_3_TIMES);  ���͵Ĵ�������һ��
{
...
void code_protocol_ack(uint8_t  x_xor,uint8_t len,uint8_t *buff,uint8_t type)  //SEND_ACKnHEX   type =0 buff Ϊhex�������ת��ASCII ;  type=1 ����ת
{
...
void COMM_check(void)
{
	...
	//if((SOUND_str.Soung_SPI_BUSY == SPI_IDLE)&&(SYSTEM_ERR_STATUS->bits.COMM_Handle_FINISH==0))//��������ʱ���ܽ���
	//{
		 ...
		 switch(COMM_STEP)
		 {
			 ...
			 break;
			 case WAIT_LEN:
				 if(UART2_RX_BUFF[RX_COUNT_OUT]!='@')//��һ���ַ�����
				 {
					#ifdef  SYSUARTPRINTF
		      sysprintf("\r\nUART_receive_err1,UART2_RX_BUFF[RX_COUNT_OUT]=%d\r\n",UART2_RX_BUFF[RX_COUNT_OUT]);
					 ...
					 COMM_STEP = COMM_IDLE;
					// code_protocol_error(ERROR_TYPE_U1,(U1A01_ERROR>>16),(int16_t)U1A01_ERROR,ERROR_TYPE_SEND_3_TIMES); // ���͵Ĵ�������				
					 return;
				 }
			 ...
			 case WAIT_ALL:
			   ...
			   if(number>=(Tp_long+3))
				 ...
				 if(get_timego(WAIT_LEN_TIMER)>100)//����ֵ38400bps 256��B ����ֵ53ms
				 {
					 RX_COUNT_OUT = (RX_COUNT_OUT+3)%RX_BUFF_MAX;//�ٳٲ���
					 COMM_STEP = COMM_IDLE;
					 code_protocol_error(ERROR_TYPE_U1,(U1A01_ERROR>>16),(int16_t)U1A01_ERROR,ERROR_TYPE_SEND_3_TIMES); // ���͵Ĵ�������				
					// SEND_DATA_COPY((uint8_t *)ERR_ILLEGAL,27);
					 return;
			 ...
			 case COMM_HANDLE:
				 ...
				 if((RX_COUNT_OUT + Tp_long+3)>RX_BUFF_MAX)
					...
					if(decode_protocol((uint8_t  *)(Tp_rx),Tp_long+3,COMM_DATA_UARST)==0)//�ӱ��ĵĵ������ֽڿ�ʼ����
					{
	...
	//}//��������ʱ���ܽ���
	//LED_LOGO_CONTROL_OFF();//LOGO LED�رյ��߼��ж�
}
...
//	  static  char dirPath[24]={0};
	static	uint32_t Tp_index;
	 ...
	 if(command_D8_D9_time)
	{
         if(buff[3]!='D')//IIC���Ĳ��� 20181108
         	{
                    //
                    //bmp_protocol.num=0; //����ȡ��
                    code_protocol_error(ERROR_TYPE_U1,(U1A02_ERROR>>16),(int16_t)U1A02_ERROR,ERROR_TYPE_SEND_3_TIMES); // ���͵Ĵ�������
	...
	switch(buff[3])
	{
		...
		case 'G':
		 ...
		 case 'P':
		     ...
		     break;
		  case 'Q':
				if(buff[9]!=','&&buff[9]!='*') return 0;//��ֹ�ļ�����1λҲ����
			  	switch(buff[4])
				{
				case '0':
					
					CpyNandToSdramCMd((buff[4]&0x0f),NULL, 0);	//���
					//�Ӷ�Ӧ���ļ�
					Tp_i=0;
					do
					{
			...
			switch(buff[4])
			{
				case '0':
				  ...
				  if(buff[17]!='*') return 0;//��ֹ�ļ�����1λҲ����
          get_vbat_ad_value(); 
				  
				  if(systerm_error_status.bits.lse_error==1)	//��ǰΪRTCû�е�
					{
						#ifdef  SYSUARTPRINTF  
						sysprintf("vat err\r\n");
            #endif						
      ...
      case 'B'://��������
			...
			if((convet_1_hex_ASCII_to_hex(buff+4)>=1)&&(convet_1_hex_ASCII_to_hex(buff+4)<=0x0A))//�������
			{
...
//			if(tts.voice<3)  // 1~9��Ӧ 1 1 1 2 3 4 5 6 7 
//				{
//					tts.voice=1;
//			      }
//			if(tts.voice>2)
...
//					}
      ...
      case 'W'://logoд
		...
		//	if(pwr_on_cnt) return 0;  //if(LOGO_HANDLE_flag == 0) return 0;//���������ݳ�ʼ��ǰ��д��������
			if(LOGO_DATA_OUT_FLAG != 3) return 0;
		//	if(pwr_on_cnt) return 0;
			...
			if((p_gs_AreaInfo->type==0)&&(filed_num !=0))  return 0;//���AreaNo���ܻ�û�б�ѧָ��
			Tp_data2 = convet_2_hex_ASCII_to_hex(buff + 1);//�����Ӧ����LEN
			if(Tp_data2 == 0xffff) return 0;
			Tp_data2 = Tp_data2 - 3; //����Ǳ���VALUE����ĳ���
			if((p_gs_AreaInfo->size!=Tp_data2)&&(filed_num !=0)) return 0;//���˵�����͵����ݸ�������   field[filed_num].number
			if((Tp_data2>11)&&(filed_num ==0)) return 0;



		     p_gs_AreaInfo=get_gs_AreaInfo(filed_num);
			led_log_on(300);// ��300ms
		  
                  //ȷ����Ŀ��
			switch(p_gs_AreaInfo->type)  //field[filed_num].data_class)
			{
				case BASE_DATA_CLASS://��������
					...
					{
						if((buff[8+n]<'A')||(buff[8+n]>'Z'))//���Ǵ�д��ĸ
						{
							if((buff[8+n]<'0')||(buff[8+n]>'9'))//��������
							{
								return 0;
							}
						}
					...
					*/
				break;
				case ACTION_NUMBER_DATA_CLASS://���������Ƕ�����������
...
//					//Tp_field = filed_num;
//					Tp_data3 =convet_3_dec_asccii_to_hex(buff + 5);//����Ƕ�����������
//					if(Tp_data3 == 0xffff) return 0;//���ݲ��Ϲ淶
//					if((Tp_data3 == 0)||(Tp_data3 == 1)) return 0;//ȡֵ��Χ1-999
//					
//					//if(sdram_actionindex >= MAX_LOGO_ACTION_MONTH) return 0 ;//���������������������ٶ���//20170510
...
//					*(__IO uint32_t*)(Tp_bmp_filename) = *(__IO uint32_t*)(Tp_bmp_filename) - 1;//��¼�ϵ���� NO Tp_data3����Ŀ			
//					*/

				break;
				case ACTION_TIMERS_DATA_CLASS://���������Ƕ���ʱ������
         	//  Tp_field = filed_num;
					 if(para.dataclass_1_2_action_count >= MAX_LOGO_ACTION_MONTH) return 0 ;//?????????????//20170510
						 Tp_data3 =convet_3_dec_asccii_to_hex(buff + 5);//?????????
					   if(Tp_data3 == 0xffff) return 0;//??????
...
//					//Tp_field = filed_num;
//					//if(sdram_actionindex >= MAX_LOGO_ACTION_MONTH) return 0 ;//����ʱ���������������ٶ���//20170510
//					Tp_data3 =convet_3_dec_asccii_to_hex(buff + 5);//����Ƕ���ʱ������
//					if(Tp_data3 == 0xffff) return 0;//���ݲ��Ϲ淶
//					if((Tp_data3 == 0)||(Tp_data3 == 1)) return 0;//ȡֵ��Χ1-999
//					//Tp_data3 = Tp_data3 +1;
//					Tp_data2 = conver_x_hex_asccii_to_hex(buff + 8,8);
//					if(Tp_data2 == 0xffffffff) return 0;//����ʱ�����ݵĺ�8λ���Ϸ� �����Զ���ʱ�����ݵĺ�8λ��Χ0-0xfffffffe
...
//					if(Tp_data3 == 0xffff) return 0;//���ݲ��Ϲ淶
//					// if(Tp_data3 == 0) return 0;//���ݲ��Ϲ淶//20170628
//					if(Tp_data3>MAX_LOGO_UINT_NUM) return 0;//ȡֵ��Χ1-MAX_LOGO_UINT_NUM
...
//					}
					...
					//int AreaWriteCmd(char cmd, int filed, int idx,void *data,int len)		//���� FILED  ���� ���� ����
					/*
					*(__IO uint8_t*)(logodata_sdrambuffer_addr_arry[filed_num] + Tp_data3) = Tp_field_addr ;
					*/
				break;
				case TEST_FINAL_DATA_DATA_CLASS://��������������
        // 	Tp_field = Tp_data1; 
...
//					*/
				break;
				case TEST_ALL_DATA_DATA_CLASS://������ȫ���ⶨ���� ������������ǿ����ظ������
...
//					}
			...
			//Updata_Basedata_No0_cyw();//20181106 ��д��ʱ�����ʱ��
			//LED_LOGO_CONTROL_ON();
				Updata_Basedata_No0_cyw();
			...
			//if(pwr_on_cnt) return 0;//if(LOGO_HANDLE_flag == 0) return 0;//���������ݳ�ʼ��ǰ���������
      if(LOGO_DATA_OUT_FLAG != 3) return 0;
			led_log_on(300);// ��300ms
...
//				    {
//			          Tp_i--;
//					      SaveAreaData(0, Tp_i);     //test ʱ�� 544ms
//				    }
//				    while(Tp_i);
						//SaveAreaData( 0);//��������0~3 ����ȡ��U��ʱ��Ҫ���ڴ�ͼ�¼ͳһһ�£�������ȡʱҪ����һ��
						usb_init();
						...
						if(i_return_flag==0)  //DOWNLOAD_LOGO_INIT(0X0000FFFF);
            {
					...
					/*
					ack_buf[0] = 0X16;//20170721 ITEM2 ָ����������
					command_xor=xor;////IMAGE_FCS = Tp_FSC;//��������
					if(type == COMM_DATA_UARST)
				...
				case '1':
					...
					//	SaveAreaData(0,filed_num );//��������0~3 ����ȡ��U��ʱ��Ҫ���ڴ�ͼ�¼ͳһһ�£�������ȡʱҪ����һ��
					//	SaveAreaData(0,0 );
						usb_init();
           ...
           i_return_flag =    BackupDeviceData( );
						...
						if(i_return_flag==0)  //DOWNLOAD_LOGO_INIT(0X0000FFFF);
            {
				 ...
				 case '1':
					//if(pwr_on_cnt) return 0;   //if(LOGO_HANDLE_flag == 0) return 0;//���������ݳ�ʼ��ǰ���������
				      if(buff[5]!='*') return 0;//��ֹ�ļ�����1λҲ����
				      //SYNCHRONOUS_BACKUP_FILE();//��ʼ�ӱ������ݵ�������״̬
		          ack_buf[0] = 0X16;//20170721 ITEM2 ָ����������
		          command_xor=Tp_xor;//IMAGE_FCS = Tp_FSC;
				      if(type == COMM_DATA_UARST)
							{
...
//							//RTC_CalibOutputCmd(ENABLE);//ʹ�ܻ����ⲿ32.768K��512HZ�������
...
////						if(convet_2_hex_ASCII_to_hex(buff+6)&0x60)    return 0;//��Щλ�ò�����ֵ
////						//RTC_CoarseCalibCmd(DISABLE);
////						//RTC_CoarseCalibConfig(convet_2_hex_ASCII_to_hex(buff+6),convet_2_hex_ASCII_to_hex(buff+6));//���ôֵ���ֵ
////						//RTC_CoarseCalibCmd(ENABLE);//ʹ�ܴ�У׼
////                                         //
...
////	   				      }
			...
			case '6':
				return 0;
//				if((buff[5]=='F')&&(buff[6]=='F')&&(buff[7]=='F'))//������ģʽ
//				{
...
//			      }
			...
			case '7':
				...
				if(AreaReadCmdF(LCD_F7, 0,Tp_data1,(char*)ack_buf, &ack_buf_len))//ack_buf Ҫ����asccii dc
				{
				    if(type == COMM_DATA_UARST)
						{
