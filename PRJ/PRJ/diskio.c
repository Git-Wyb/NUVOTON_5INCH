...
static BYTE  fatfs_win_buff_pool[FF_MAX_SS] __attribute__((aligned(32)));       /* FATFS window buffer is cachable. Must not use it directly. */
BYTE  *fatfs_win_buff;
...
////////////////////////	RTC_Read(RTC_CURRENT_TIME,&timeandday_now);
	...
	//Tp_now.year = ((timeandday_now.u32Year-RTC_YEAR2000)&0xff);//(Tp_now.year/16)*10 +  Tp_now.year%16;
	//Tp_now.month = (Tp_now.month/16)*10 +  Tp_now.month%16;
	//Tp_now.day = (Tp_now.day/16)*10 +  Tp_now.day%16;
	//Tp_now.hour = (Tp_now.hour/16)*10 +  Tp_now.hour%16;
	//Tp_now.minute = (Tp_now.minute/16)*10 +  Tp_now.minute%16;
	//Tp_now.second = (Tp_now.second/16)*10 +  Tp_now.second%16;
	
	return (((HEXTODEC(timeandday_now_app.year)+20)&0xff) << 25) /* Year = 2010 */
| (HEXTODEC(timeandday_now_app.month&0xff) << 21) /* Month = 11 */
| (HEXTODEC(timeandday_now_app.day&0xff) << 16) /* Day = 2 */
| (HEXTODEC(timeandday_now_app.hour&0xff) << 11) /* Hour = 15 */
| (HEXTODEC(timeandday_now_app.minute&0xff) << 5) /* Min = 0 */
| (HEXTODEC(timeandday_now_app.minute&0xff) >> 1) /* Sec = 0 */
;
}	
