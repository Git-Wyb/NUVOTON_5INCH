...
static int  do_scsi_command(MSC_T *msc, uint8_t *buff, uint32_t data_len, int bIsDataIn, int timeout_ticks)
{
    ...
    ret = msc_bulk_transfer(msc, msc->ep_bulk_out, (uint8_t *)cmd_blk, 31, timeout_ticks/*timeout_ticks*/);
    if (ret < 0)
		{
      sysprintf("msc_bulk_transfer0=%X\r\n",ret);  
			return ret;
		}
    ...
    {
        if (bIsDataIn)
            ret = msc_bulk_transfer(msc, msc->ep_bulk_in, buff, data_len, 50000);
        else
            ret = msc_bulk_transfer(msc, msc->ep_bulk_out, buff, data_len, 50000);
        if (ret < 0)
				{
            sysprintf("msc_bulk_transfer1=%X\r\n",ret);  
					  return ret;
				}
        msc_debug_msg("    [XFER] MSC DATA OK.\n");
    }

    ret = msc_bulk_transfer(msc, msc->ep_bulk_in, (uint8_t *)cmd_status, 13, 50000);
    if (ret < 0)
		{
        sysprintf("msc_bulk_transfer2=%X\r\n",ret);  
			  return ret;
		}
