...
UINT32 volatile _sys_uTimer0Count = 0, _sys_uTimer1Count = 0,time1ms_count;
extern UINT32 volatile time1ms_count_forrun;
...
void protocol_D8_D9_command_time_dealwith(void) ;
extern E_INT_STATUS tim1ms_control;
...
{
    int volatile i;
    if(tim1ms_control==int_disable)
	   {
	          set_tim1msint(int_wait);
	           return;	
	   }
    /*----- check channel 0 -----*/
...
INT32 sysStartTimer(INT32 nTimeNo, UINT32 uTicksPerSecond, INT32 nOpMode)
{
    ...
    switch (nTimeNo)
    {
        case TIMER0:
            ...
            _sys_pvOldTimer0Vect = sysInstallISR(HIGH_LEVEL_SENSITIVE | IRQ_LEVEL_4, TMR0_IRQn, (PVOID)sysTimer0ISR);
