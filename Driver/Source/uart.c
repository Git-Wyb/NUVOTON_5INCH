...
static UINT32 UARTTXBUFSIZE[UART_NUM] = {500, 1024,500, 500, 500, 500, 500, 500, 500, 500, 500};  /* UART0~10 Tx buffer size */
static UINT32 UARTRXBUFSIZE[UART_NUM] = {500, 2048,500, 500, 500, 500, 500, 500, 500, 500, 500};    /* UART0~10 Rx buffer size */
...
extern volatile  uint32_t UART_TX_REAL;
...
static void _uartInstallISR(UINT8 ucNum)
{
    ...
    if(ucNum == UART0) {
        IRQ= UART0_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart0ISR);
    } else if(ucNum == UART1) {
        IRQ= UART1_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart1ISR);
    } else if(ucNum == UART2) {
        IRQ= UART2_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart2ISR);
    } else if(ucNum == UART3) {
        IRQ= UART3_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart3ISR);
    } else if(ucNum == UART4) {
        IRQ= UART4_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart4ISR);
    } else if(ucNum == UART5) {
        IRQ= UART5_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart5ISR);
    } else if(ucNum == UART6) {
        IRQ= UART6_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart6ISR);
    } else if(ucNum == UART7) {
        IRQ= UART7_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart7ISR);
    } else if(ucNum == UART8) {
        IRQ= UART8_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart8ISR);
    } else if(ucNum == UART9) {
        IRQ= UART9_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart9ISR);
    } else if(ucNum == UARTA) {
        IRQ= UART10_IRQn;
        dev->pvUartVector = sysInstallISR((IRQ_LEVEL_3 | HIGH_LEVEL_SENSITIVE), IRQ, (PVOID)uart10ISR);
    } else {
        return;
    }
...
static BOOL _uartBUFSpaceAlloc(INT nNum)
{
    ...
    dev->pucUartTxBuf = (PUINT8) malloc(UARTTXBUFSIZE[nNum] * sizeof(UINT8)+64);
    if(dev->pucUartTxBuf == NULL)
        return FALSE;

    /* Memory allocate Rx buffer */
    dev->pucUartRxBuf = (PUINT8) malloc(UARTRXBUFSIZE[nNum] * sizeof(UINT8)+64);
    ...
    dev->pucUARTFlag = (PINT) malloc(UARTRXBUFSIZE[nNum] * sizeof(INT)+64);
